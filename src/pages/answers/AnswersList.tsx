...
type SortKey = "text" | "isCorrect" | "createdAt";
...
export default function AnswersList() {
  ...
  const { data, loading, error, refetch, setData } = useFetch<{ items: Answer[]; total: number }>("/answers", { query });
  ...
  const openCreate = () => {
    ...
    const isCorrect = (prompt("Is correct? (true/false):", "false") || "false").toLowerCase() === "true";
    answerApi.create({ text, questionId, isCorrect }).then(() => refetch());
  };

  const openEdit = (entity: Answer) => {
    const text = prompt("Answer text:", entity.text) || entity.text;
    const questionId = prompt("QuestionId:", entity.questionId) || entity.questionId;
    const isCorrect = (prompt("Is correct? (true/false):", String(entity.isCorrect)) || String(entity.isCorrect)).toLowerCase() === "true";
    answerApi.update(entity._id!, { text, questionId, isCorrect }).then(() => refetch());
  };

  const onDelete = (id: string) => {
    if (!confirm("Delete this answer?")) return;
    answerApi.remove(id).then(() => {
      ...
      <div className="overflow-x-auto bg-white shadow rounded-lg">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {["text", "isCorrect", "createdAt"].map((key) => (
                <th
                  key={key}
          ...
          <tbody className="divide-y divide-gray-200">
            {(data?.items || []).map((a) => (
              <tr key={a._id}>
                <td className="px-6 py-4 whitespace-nowrap">{a.text}</td>
                <td className="px-6 py-4 whitespace-nowrap">{a.isCorrect ? "Yes" : "No"}</td>
