...
type SortKey = "totalScore" | "startedAt" | "finishedAt";
...
export default function AttemptsList() {
  ...
  const { data, loading, error, refetch, setData } = useFetch<{ items: Attempt[]; total: number }>("/attempts", { query });
    ...
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          <input
            placeholder="Search user/assessment..."
            value={search}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              {["totalScore", "startedAt", "finishedAt"].map((key) => (
                <th
                  key={key}
          ...
          <tbody className="divide-y divide-gray-200">
            {(data?.items || []).map((at) => (
              <tr key={at._id}>
                <td className="px-6 py-4 whitespace-nowrap">{at.userId}</td>
                <td className="px-6 py-4 whitespace-nowrap">{at.assessmentId}</td>
                <td className="px-6 py-4 whitespace-nowrap">{at.totalScore}</td>
                <td className="px-6 py-4 whitespace-nowrap">{at.startedAt ? formatDate(at.startedAt) : "-"}</td>
                <td className="px-6 py-4 whitespace-nowrap">{at.finishedAt ? formatDate(at.finishedAt) : "-"}</td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex justify-end gap-2">
                  <button onClick={() => onDelete(at._id!)} className="text-red-600 hover:text-red-900">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
